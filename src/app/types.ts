...
export interface FeedingRecord { id:string; date:string; month:string; year:number; pond:string; brand:string; size:string; morning:number; evening:number; total:number; recordedBy:string; morningTime?:string; eveningTime?:string; createdBy?:string; createdById?:string; editHistory?:FeedEditEntry[]; }
export interface MortalityEntry { id:string; pondId:string; date:string; count:number; cause:string; notes:string; }
export interface BagOpenLog    { id:string; date:string; month:string; year:number; brand:string; size:string; kgPerBag:number; bagsOpened:number; totalKg:number; fishStock?:string; }
export interface FeedRemainingLog { id:string; brand:string; size:string; fishStock:string; remainingKg:number; date:string; }
...
export interface StockEvent   { id:string; pondId:string; pondName:string; date:string; species:string; count:number; avgWeight?:number; cost:number; salePrice?:number; type:"Initial"|"Transfer"|"Restock"|"Closed"; fromPond?:string; clearedDate?:string; supplier?:string; }
