...
export interface StaffMember  { id:string; userId?:string; staffAuthId?:string; name:string; email:string; phone:string; role:string; status:"Active"|"Pending"; joinedDate:string; permissions:string[]; farms:string[]; }
...
export interface UserProfile  { id?:string; name:string; farmName:string; city:string; state:string; country:string; email:string; phone:string; currencySymbol:string; currencyCode:string; activePlan?:string|null; trialStartDate?:string|null; role?:string; permissions?:string[]; ownerId?:string; }
